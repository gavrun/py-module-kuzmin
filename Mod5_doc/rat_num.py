"""
Implementation of the Rational Number (RatNum) ADT
"""
...
class RatNum:
    ...
    Attributes:
        ...
        NaN A constant holding a Not-a-Number (NaN) value of type RatNum
        ZERO A constant holding a zero value of type RatNum
    ...
    def __init__(self, num : int, den : int = 1):
        ...
        */
        ...
        # special case for zero denominator; gcd(num, den) requires den != 0
        if den not in (0, 1):
            # reduce ratio to lowest terms
            gc_divisor = RatNum.gcd(num, den)
            num = num / gc_divisor
            den = den / gc_divisor
    ...
    // Throws a RuntimeException if the rep invariant is violated.
        ...
        if self.denom < 0:
            raise RuntimeError("Denominator of a RatNum cannot be less than zero")

        if self.denom > 0:
            this_gcd = RatNum.gcd(self.numer, self.denom)
            if this_gcd not in (-1, 1):
                raise RuntimeError("RatNum not in the lowest form")
